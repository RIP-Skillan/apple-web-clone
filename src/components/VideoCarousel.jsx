        ...
        {hightlightsSlides.map((list,i) => (
          <div key={list.id} id='slider' className='sm=pr-20 pr-10'>
            <div className='video-carousel_container'>
              <div className='w-full flex-center h-full rounded-3xl overflow-hidden bg-black'>
                <video 
                  id='video' 
                  playsInline={true} 
                  preload='auto' 
      ...
      <div className='relative flex-center mt-10'>
        <div className='flex-center py-5 px-7 bg-gray-800 backdrop-blur rounded-full'>
          {videoRef.current.map((_,i)=>(
            <span key={i} ref={(el) => (videoDivRef.current[i] = el)} className='mx-2 w-3 h-3 bg-gray-700 rounded-full relative cursor-pointer'>
